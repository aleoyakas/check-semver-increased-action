...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const checkIsValidSemVer_1 = __importDefault(__nccwpck_require__(2999));
const getSummary = (isValid) => isValid ? 'is valid' : 'is not valid';
const checkInputs = (currentVersion, previousVersion, allowPreRelease) => {
    const isCurrentVersionValid = (0, checkIsValidSemVer_1.default)(currentVersion, allowPreRelease);
    const isPreviousVersionValid = (0, checkIsValidSemVer_1.default)(previousVersion, true);
    if (isCurrentVersionValid && isPreviousVersionValid) {
        return { isValid: true };
    }
    return {
        isValid: false,
        summary: `currentVersion ${getSummary(isCurrentVersionValid)}. previousVersion ${getSummary(isPreviousVersionValid)}.`
    };
};
...
const checkIsValidSemVer = (version, allowPreRelease) => {
    if (!allowPreRelease && !!semver_1.default.prerelease(version)) {
        return false;
    }
    return !!semver_1.default.valid(version);
};
exports["default"] = checkIsValidSemVer;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const semver_1 = __importDefault(__nccwpck_require__(1383));
const compareVersions = (currentVersion, previousVersion) => semver_1.default.gt(currentVersion, previousVersion);
...
const checkInputs_1 = __importDefault(__nccwpck_require__(7378));
const compareVersions_1 = __importDefault(__nccwpck_require__(624));
async function run() {
    try {
        const currentVersion = core.getInput('current-version');
        const previousVersion = core.getInput('previous-version');
        const allowPreRelease = core.getInput('allow-pre-release').toLowerCase() === 'true';
        const { isValid, summary } = (0, checkInputs_1.default)(currentVersion, previousVersion, allowPreRelease);
        if (!isValid) {
            core.setFailed(summary);
        }
        const isVersionIncreased = (0, compareVersions_1.default)(currentVersion, previousVersion);
